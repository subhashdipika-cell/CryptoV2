import { calculateExitLevels } from "@/lib/bot-position";
...
export async function GET(){
 try{
  ...
  const positions=await Promise.all(owned.map(async({currency,position})=>{const [tickerRaw,instrument]=await Promise.all([getTicker(position.instrument_name),getInstrument(position.instrument_name)]);const ticker=tickerRaw as Ticker;const markPrice=ticker.mark_price??position.mark_price;return{currency,instrumentName:position.instrument_name,direction:position.direction,size:position.size,averagePrice:position.average_price,averagePriceUsd:position.average_price_usd??position.average_price*position.index_price,markPrice,indexPrice:ticker.index_price??position.index_price,markIv:ticker.mark_iv??null,bestBid:ticker.best_bid_price??null,bestAsk:ticker.best_ask_price??null,floatingPnl:position.floating_profit_loss,floatingPnlUsd:position.floating_profit_loss_usd??position.floating_profit_loss*position.index_price,totalPnl:position.total_profit_loss,expirationTimestamp:instrument.expiration_timestamp,greeks:{delta:ticker.greeks?.delta??position.delta??null,gamma:ticker.greeks?.gamma??position.gamma??null,vega:ticker.greeks?.vega??position.vega??null,theta:ticker.greeks?.theta??position.theta??null,rho:ticker.greeks?.rho??null},exit:calculateExitLevels(position.average_price,markPrice,config.stopLossPct,config.takeProfitPct)};}));
