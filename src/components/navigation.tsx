...
export function Sidebar({ active, onChange, open, onClose }: { active: TabId; onChange: (id: TabId) => void; open: boolean; onClose: () => void }) {
  return <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[224px] flex-col border-r border-line/80 bg-[#080c10]/95 backdrop-blur-xl transition-transform lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
    <div className="flex h-[68px] items-center justify-between border-b border-line/70 px-5">
      <div className="flex items-center gap-2.5"><div className="relative grid size-8 place-items-center"><Hexagon className="absolute size-8 text-mint" strokeWidth={1.4}/><span className="font-mono text-xs font-black text-mint">K</span></div><div><div className="text-sm font-bold tracking-[.16em]">KRYPTO</div><div className="font-mono text-[8px] uppercase tracking-[.28em] text-slate-500">Terminal</div></div></div>
    ...
    <div className="border-t border-line/70 p-3"><div className="mb-2 rounded-md border border-mint/10 bg-mint/[.035] p-3"><div className="mb-2 flex items-center gap-2"><span className="pulse-dot size-1.5 rounded-full bg-mint"/><span className="font-mono text-[9px] uppercase tracking-wider text-mint">Paper engine online</span></div><div className="text-[10px] leading-relaxed text-slate-600">Market feeds simulated<br/>Order routing locked</div></div><button className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-xs text-slate-500 hover:text-slate-200"><Settings2 className="size-4"/>Settings</button></div>
  </aside>;
}
...
export function Header({ onMenu, title }: { onMenu: () => void; title: string }) {
  return <header className="sticky top-0 z-30 flex h-[68px] items-center justify-between border-b border-line/70 bg-ink/85 px-4 backdrop-blur-xl sm:px-6">
    <div className="flex items-center gap-3"><button onClick={onMenu} className="text-slate-400 lg:hidden"><Menu className="size-5"/></button><div><div className="text-[11px] text-slate-500">Terminal / <span className="text-slate-300">{title}</span></div><h1 className="text-lg font-semibold tracking-tight">Digital Asset Intelligence</h1></div></div>
